...
class ResizableImage:
    def __init__(self, frame, img_array=None, plot_func=None, data=None, text=None):
        self.frame = frame
    ...
    def resize_plot(self, width, height):
        ...
        self.plot_func(fig, ax, self.data, self.text)
...
def create_resizable_content(frame, img_array, plot_func, data, row, col, text):
    resizable_content = ResizableImage(frame, img_array, plot_func, data, text)
...
def plot_func1(fig, ax, data, text):
    data = np.array(data)   
...
def plot_func2(fig, ax, data, text):
    data = np.array(data)    
    for i in range(data.shape[1]):
        ax.plot(data[:, i], label=f'EE Vector {i+1}')
    ax.legend()
    ax.set_xlabel('Timesteps')
...
def plot_func3(fig, ax, data, title):
    data = np.array(data)    
    for i in range(data.shape[1]):
        if i == 0:
            label = 'text'
...
def plot_func4(fig, ax, attention_data, text):
...
def load_pickle_file():
    ...
    try:
        ...
        create_resizable_content(frame4, None, plot_func3, data["features_gradient_weighted_tokens"], 0, 0,"Grad Weighted Action Tokens w.r.t Action Features")
        create_resizable_content(frame4, None, plot_func3, data["action_gradient_weighted_tokens"], 0, 1, "Grad Weighted Action Tokens w.r.t Action")
...
def update_display(value):
    global data
    i = int(value)
    create_resizable_content(frame1, None, plot_func4, data["generic_atten_wrt_action_gradient"][i], 0, 0,"Generic Attention Map across all layers")
    create_resizable_content(frame1, None, plot_func4, data["last_layer_atten_maps"][i], 0, 1,"Last Layer Attention Map")
    ...
    frame3.grid(row=0, column=0, padx=5, pady=5,sticky='nsew')
...
def ask_for_folder_name():
    root = tk.Tk()
    root.withdraw()# Hide the main window
    folder_name = simpledialog.askstring("Input", "Enter the name of the new folder:")
    root.destroy()
    root.update()
    return folder_name
...
def save_frame_container_as_image():
    ...
    new_folder_path = create_folder(directory, folder_name)
    if not new_folder_path:
        messagebox.showerror("Error", "Failed to create the folder.")
        return

    time.sleep(1)
   
    frames = [frame_container, frame1, frame2, frame3, frame4, text_widget]
    # Define the save path within the new folder

    for i, frame in enumerate(frames):
        if i<5:
            image = capture_frame(frame)
            save_path = os.path.join(new_folder_path, f"frame_{i+1}.png")
            image.save(save_path)
        else:
            text_content = frame.get("1.0", tk.END)
            file_path = os.path.join(new_folder_path, "text.txt")
            with open(file_path, 'w') as file:
                file.write(text_content)
                
    messagebox.showinfo("Save Successful", f"Images and text saved successfully in {new_folder_path}")
...
frame_container1 = ttk.Frame(frame_container, relief= tk.GROOVE)
frame_container2 = ttk.Frame(frame_container, relief= tk.GROOVE)
frame_container1.grid(row=0, column=0, sticky="nsew")
frame_container2.grid(row=0, column=1, sticky="nsew")

# Create three frames within the main container frame
frame1 = ttk.Frame(frame_container1, relief= tk.GROOVE)
frame2 = ttk.Frame(frame_container2, relief= tk.GROOVE)
frame3 = ttk.Frame(frame_container2, relief= tk.GROOVE)
frame4 = ttk.Frame(frame_container1, relief= tk.GROOVE)
text_widget = tk.Text(frame_container2, relief= tk.GROOVE)

#frame_container.rowconfigure(1, weight=1)
#frame_container.rowconfigure(2, weight=1)

frame_container1.rowconfigure(0, weight=1)
frame_container1.rowconfigure(1, weight=1)
frame_container1.columnconfigure(0, weight=1)

frame_container2.rowconfigure(0, weight=2)
#frame_container2.rowconfigure(2, weight=1)
frame_container2.columnconfigure(0, weight=1)
...
action_button = ttk.Button(main_frame, text="Action Tokens", command= lambda: open_plot_window('action_gradient_weighted_tokens'))
action_button.grid(row=3, column=3, padx=5)

feat_button = ttk.Button(main_frame, text="Features Tokens", command=lambda: open_plot_window('features_gradient_weighted_tokens'))
...
slider.grid(row=3, column=0, columnspan=2, padx=5, sticky='ew')
